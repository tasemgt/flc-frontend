...
export default function EditConsumer() {
  ...
  useEffect(() => {
    if (consumer && consumer.isActive === false) {
      navigate(`${basePath}/consumers/${id}`, { replace: true })
    }
  }, [consumer])
                  ...
                  <div style={{ display: 'flex', gap: 20, marginTop: 10 }}>
                    {['yes', 'no'].map((val) => (
                      <label key={val} style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: canEdit('behaviorPlan') ? 'pointer' : 'not-allowed', fontSize: 14, color: '#555' }}>
                        <input type="radio" name="behaviorPlan" value={val} defaultChecked={consumer.behaviorPlan === val} disabled={!canEdit('behaviorPlan')} />
